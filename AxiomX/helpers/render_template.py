...
from AxiomX import pbot as bot
from AxiomX.helpers.pyro_utils import humanbytes
from AxiomX.helpers.pyro_utils import get_file_ids
...
async def render_page(message_id, secure_hash):
    try:
        ...
        if str(file_data.mime_type.split('/')[0].strip()) == 'video':
            template_path = 'AxiomX/helpers/template/req.html'
            heading = f'Watch {file_name}'
        elif str(file_data.mime_type.split('/')[0].strip()) == 'audio':
            template_path = 'AxiomX/helpers/template/req.html'
            heading = f'Listen {file_name}'
        else:
            template_path = 'AxiomX/helpers/template/dl.html'
