...
async def _discover_and_refresh_modules(bot, chat_id: int, user_id: int = None) -> Dict[str, str]:
    ...
    for module_name, module in list(sys.modules.items()):
        is_plugin = module_name.startswith('AxiomX.plugins.')
        is_decorator = module_name == 'AxiomX.helpers.decorator'
...
async def smart_cache_refresh(bot, chat_id: int, user_id: int = None, client=None) -> Dict[str, str]:
    ...
    try:
        if 'AxiomX.plugins.admins' in sys.modules:
            from AxiomX.plugins.admins import (
                get_chat_cached, 
                get_member_cached, 
                get_admin_cached,
                reload_admins_cache
    ...
    try:
        if 'AxiomX.helpers.decorator' in sys.modules:
            from AxiomX.helpers.decorator import (
    ...
    plugin_modules = sorted([
        ...
        if name.startswith('AxiomX.plugins.') and sys.modules[name] is not None
    ])
