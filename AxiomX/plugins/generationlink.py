...
from AxiomX import pbot, DEV_LIST, font
from AxiomX import database2 as database
...
async def generate_link_cmd(_, message: Message):
    ...
    if not chat:
        return await message.reply_text(font("⚠️ Chat not found in database."), quote=True)
