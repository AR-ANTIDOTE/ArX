...
from AxiomX import aiohttpsession as session, BOT_USERNAME, telegraph, app, LOGGER, LOGS_CHANNEL, font
from AxiomX.helpers.decorator import Command, send_action, Callbacks, spam_control, only_premium
from AxiomX.helpers.scripts import AiChats, paste, Gemini, GPTGeneration
from AxiomX.helpers.utils import get_ua, UserId, get_as_document
...
from AxiomX.helpers.scripts import get_output
import json
import tempfile
import contextlib
import config
...
*Commands:*  
❂ `/AxiomX <query>` - Use Makima AI (supports photos and stickers).  
...
async def DrawImg(update: Update, context: CallbackContext):
    ...
    if len(m.text.split()) < 2:
        return await m.reply_text(font("🙋 where prompt ? e.g /imagine anime AxiomX"))
...
@spam_control
async def AxiomXAi(update: Update, context: CallbackContext) -> None:
    ...
    if len(message.text.split()) == 1:
        return await msg.edit_text(
            "*Usage:* `/AxiomX <your question>`\n\n"
            "Example:\n`/AxiomX recommend anime to watch`",
            parse_mode=constants.ParseMode.MARKDOWN
        )
    ...
    reply_text = data.get('reply', "I'm AxiomX Ackerman.")
