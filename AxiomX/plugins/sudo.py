...
import config
from AxiomX.helpers.decorator import Command, sudos_only, owner_only
from AxiomX.db.sudo import add_sudo_user, remove_sudo_user, get_all_sudo_users, add_whitelist_user, remove_whitelist_user, get_all_whitelist_users, add_support_user, remove_support_user, get_all_support_users
...
async def whitelist_list(update, context):
    ...
    parts = ["📝 <b>AxiomX's Whitelisted Users</b>:\n\n"]
...
async def support_list(update, context):
    ...
    parts = ["🙋 <b>AxiomX's Support Users</b>:\n\n"]
    for user_id in users:
        parts.append(f"• {await get_user_info(context, user_id)}\n")
    await update.message.reply_text("".join(parts), parse_mode=constants.ParseMode.HTML)
