...
from AxiomX.db.users import update_user_premium, get_all_premium_users
...
async def update_premium_func(update, context):
    ...
    try:
        ...
        if action == "-add":
            ...
            text = "✅ *AxiomX's Premium successfully granted to the user.*" if success else "🧐 *User is already premium.*"
            ...
            text = "❌ *AxiomX's Premium removed from the user.*" if success else "🧐 *User is not a premium user.*"
