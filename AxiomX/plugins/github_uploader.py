...
from AxiomX import app, prefix_cmds, BOT_USERNAME, font
from AxiomX.helpers.decorator import Command, sudos_only, only_private
...
async def gitupload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        user = g.get_user()
        repo = user.create_repo(repo_name, private=is_private, description="sᴏᴜʀᴄᴇ ᴄᴏᴅᴇ ᴜᴘʟᴏᴀᴅ ʙʏ :- ᴧꭙ‌ɪᴏϻ ᴄᴏᴅᴇʀ", auto_init=False)
        ...
        if status_out.stdout.strip():
            run(["git", "commit", "-m", "ᴜᴘʟᴏᴀᴅᴇᴅ-ʙʏ-ᴧꭙ‌ɪᴏϻ-ᴄᴏᴅᴇʀ"], cwd=final_path)
        else:
            run(["git", "commit", "--allow-empty", "-m", "ɴᴇᴡ-ᴜᴘᴅᴀᴛᴇ-ʙʏ-ᴧꭙ‌ɪᴏϻ-ᴄᴏᴅᴇʀ"], cwd=final_path)
